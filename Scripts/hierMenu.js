...
function HM_f_GetItemDivStr(itemid,disptext,hasmore){
	...
	var TempString = "<DIV ID=" + itemid + " STYLE='position:absolute;text-align:center;width:" + WidthValue + "px'>";
...
function HM_f_ItemOut() {
    ...
    if ( (!HM_KeepHilite || ((this.tree.TopIsPermanent && (this.tree.treeParent==this)) && !this.menu.hasChildVisible)) && this.hasRollover) {
        if(HM_NS4) {
			bgColor = this.tree.BGColor;
			this.menu.currentItem.txtLyrOff.visibility = "inherit";
			this.menu.currentItem.txtLyrOn.visibility = "hide";
		}
		else {
			with(this.style) {
				backgroundColor = this.tree.BGColor;
...
function HM_f_ShowIt(on) {
	if (!(this.tree.TopIsPermanent && (this.tree.treeParent==this))) {
		...
		if(HM_NS4) {this.visibility = (on) ? "show" : "hide";
					this.cursor = (on) ? "hand" : "default"}
		else {this.style.visibility = (on) ? "visible" : "hidden";
			  this.style.cursor = ('default') ? "hand" : "default"}
	}
